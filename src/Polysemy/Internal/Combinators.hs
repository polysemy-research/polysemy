...
firstOrder
    ...
    -> t
firstOrder higher f = higher $ \(e :: e m x) ->
  liftT $ f e
{-# INLINE firstOrder #-}
...
intercept
    ...
    -> Sem r a
intercept f = interceptH $ \(e :: e m x) ->
  liftT @m $ f e
{-# INLINE intercept #-}
...
interceptUsing
    ...
    -> Sem r a
       -- ^ Unlike 'interpret', 'intercept' does not consume any effects.
    -> Sem r a
interceptUsing pr f = interceptUsingH pr $ \(e :: e m x) ->
  liftT @m $ f e
